...
! 1 = generate the whole mesh   2 = generate only half the mesh
  integer, parameter :: factor_divide_mesh = 1
...
! 90 is accurate down to periods of about 0.5 second
  integer, parameter :: multiplication_factor = 5 ! 1 ! 22 ! 45
...
! very large and very small values
  ...
  integer :: num_surf
  integer, dimension(nspec_surf_whole_circle) :: elem_id_surf
...
!---- volume
  ...
  do irad = 0,nspec_rad_670_surf ! irad=nspec_rad_670_surf is the surface
...
! %%% bloc d670 -> surface
  do irad = 0,nspec_rad_670_surf-2,2 ! irad=nspec_rad_670_surf is the surface
  do ix = 0,nspec_surf_whole_circle/factor_divide_mesh-2,2
      ...
      if (irad == nspec_rad_670_surf-2) then ! element on the surface

        num_surf = num_surf+1
        
        ! write(*,*) "element number ", num_surf, ispec, elem_id_surf(num_surf), "on the surface before 2"

        elem_id_surf(num_surf) = 0

        ! write(*,*) "element number ", num_surf, ispec, elem_id_surf(num_surf), "on the surface before"
        
        elem_id_surf(num_surf) = ispec

        ! write(*,*) "element number ", num_surf, ispec, elem_id_surf(num_surf), "on the surface"

        
      endif

      ! write(*,*) "element number ", num_surf, ispec, elem_id_surf(num_surf), "on the surface after"
  enddo
  enddo
...
! count the number of elements that are in contact with the symmetry axis
  do ispec = 1,nspec
    ...
    if (abs(sqrt(xcoord(1,ispec)**2 + ycoord(1,ispec)**2) - R_EARTH) < 5) i = i + 1
    if (abs(sqrt(xcoord(2,ispec)**2 + ycoord(2,ispec)**2) - R_EARTH) < 5) i = i + 1
    if (abs(sqrt(xcoord(3,ispec)**2 + ycoord(3,ispec)**2) - R_EARTH) < 5) i = i + 1
    if (abs(sqrt(xcoord(4,ispec)**2 + ycoord(4,ispec)**2) - R_EARTH) < 5) i = i + 1
...
! and a single edge can of course be detected otherwise that element would have a 180-degree angle
      ...
      if (i /= 2) then
        ! write(*,*) ispec
        stop 'error: element in contact with the surface by incorrect number of points'
      endif
      num_surf = num_surf + 1
    endif
  enddo
...
open(unit=22,file='MESH/Surf_free_AK135F_NO_MUD',form='formatted',status='unknown',action='write')
  ...
  do ispec = 1,nspec
#ifdef USE_BINARY_FOR_EXTERNAL_MESH_DATABASE
    if (abs(sqrt(xcoord(1,ispec)**2 + ycoord(1,ispec)**2) - R_EARTH) < 5 .and. &
      abs(sqrt(xcoord(2,ispec)**2 + ycoord(2,ispec)**2) - R_EARTH) < 5) write(22) ispec,' 2 ', &
      ibool(1,ispec),ibool(2,ispec)
    if (abs(sqrt(xcoord(2,ispec)**2 + ycoord(2,ispec)**2) - R_EARTH) < 5 .and. &
      abs(sqrt(xcoord(3,ispec)**2 + ycoord(3,ispec)**2) - R_EARTH) < 5) write(22) ispec,' 2 ', &
      ibool(2,ispec),ibool(3,ispec)
    if (abs(sqrt(xcoord(3,ispec)**2 + ycoord(3,ispec)**2) - R_EARTH) < 5 .and. &
      abs(sqrt(xcoord(4,ispec)**2 + ycoord(4,ispec)**2) - R_EARTH) < 5) write(22) ispec,' 2 ', &
      ibool(3,ispec),ibool(4,ispec)
    if (abs(sqrt(xcoord(4,ispec)**2 + ycoord(4,ispec)**2) - R_EARTH) < 5 .and. &
      abs(sqrt(xcoord(1,ispec)**2 + ycoord(1,ispec)**2) - R_EARTH) < 5) write(22) ispec,' 2 ', &
      ibool(4,ispec),ibool(1,ispec)
#else
    if (abs(sqrt(xcoord(1,ispec)**2 + ycoord(1,ispec)**2) - R_EARTH) < 5 .and. &
      abs(sqrt(xcoord(2,ispec)**2 + ycoord(2,ispec)**2) - R_EARTH) < 5) write(22,*) ispec,' 2 ', &
      ibool(1,ispec),ibool(2,ispec)
...
! and a single edge can of course be detected otherwise that element would have a 180-degree angle
      if (i == 1) then
        write(*,*) ispec, i
        stop 'error: element in contact with the symmetry axis by incorrect number of points'
      endif
