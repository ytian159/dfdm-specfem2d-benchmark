...
namespace DFDM{
    ...
    class matrix{
        public:
            ...
            uint32_t rows, cols;
            int kd = -1; // -1 by default, only used for banded matrix
            bool banded = false; // false by default
            matrix();
            ...
            void alt_factorisation(matrix& matl, matrix& matlt, uint64_t p);
            void fact_lower_diag_twist(int j, matrix& L, int isplit);
            void fact_mass_twist(matrix& matl, matrix& matlt, uint64_t p);
            void fact_mass_twist_1(matrix& matl, matrix& matlt, uint64_t p);
            matrix vecprod(const matrix& vec);
            matrix matprod(const matrix& matI) const;
            static void triangular_solve(const matrix& matL, matrix& matX, matrix& matB, const char* uplo);
            static void banded_triangular_solve(const matrix& matL, matrix& matX, matrix& matB, const char* uplo);
            matrix toBanded(int kd, const char* uplo);
            void print();
