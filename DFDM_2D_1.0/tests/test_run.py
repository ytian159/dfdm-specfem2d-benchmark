...
output_path = os.path.join(cwd,"tests/test_out/")
stdout_capture = os.path.join(cwd,"tests/test_out/test_1.out")
reference = os.path.join(dfdm_dir, "tests/full_sim_reference/")
domain_out = os.path.join(cwd, "tests/test_out/domain/")
...
cmd = exe+str(" -c ")+config_path+str(" -o ")+output_path+str(" -m ")+domain_in+str(" -d ")+domain_out+str(" ")+">> "+stdout_capture
