...
- Elements with centroid radius < R_CUT (slightly above R_TARGET to include
  the outermost element layer)
- Outer boundary nodes snapped radially to R_TARGET (matching DFDM boundary)
- Absorbing boundary at the outer edge (R_TARGET)
- No free surface
...
# Configuration
# ============================================================
R_CUT = 1701.0e3  # meters - raised to include elements straddling 1700 km
R_TARGET = 1700.0e3  # meters - snap outer nodes to match DFDM boundary (rads[2] = 1700 km)
...
def main():
    print("=" * 60)
    print(f"Cutting mesh to R < {R_CUT/1e3:.0f} km, snapping boundary to {R_TARGET/1e3:.1f} km")
    print("=" * 60)
    ...
    # Remap material IDs: mesh generator uses IREGION_OUTER_CORE=3, IREGION_INNER_CORE=4
    # but Nummaterial_velocity_file_1700km defines materials 1 and 2.
    remap = {3: 1, 4: 2}
    for old_id, new_id in remap.items():
        count = np.sum(kept_mat == old_id)
        if count > 0:
            print(f"    Remapping material {old_id} -> {new_id} ({count} elements)")
            kept_mat[kept_mat == old_id] = new_id
    ...
    print(f"  Radius range before rescale: [{new_r.min()/1e3:.1f}, {new_r.max()/1e3:.1f}] km")

    # ---- Find boundary edges (needed before rescaling) ----
    ...
    for elem_id, edge_type, n1, n2 in boundary_edges:
        ...
        if avg_r > 0.5 * R_CUT:  # outer boundary
            ...
            inner_edges.append((elem_id, edge_type, n1, n2))

    print(f"  Total boundary edges: {len(boundary_edges)}")
    print(f"  Absorbing boundary edges (outer): {len(abs_edges)}")

    if inner_edges:
        print(f"  Inner boundary edges: {len(inner_edges)} (central cube)")

    # ---- Radial compression of boundary element layer ----
    # Find inner face radius of outer boundary elements by looking at the
    # opposite edge of each boundary element
    inner_corner_ids = set()
    for elem_id, edge_type, n1, n2 in abs_edges:
        elem_idx = elem_id - 1  # 0-indexed
        conn_row = new_conn[elem_idx]
        opp_edge = OPPOSITE_EDGE[edge_type]
        ci, cj = EDGES[opp_edge]
        inner_corner_ids.add(conn_row[ci])
        inner_corner_ids.add(conn_row[cj])

    inner_radii = np.array([np.sqrt(new_coords[n - 1, 0]**2 + new_coords[n - 1, 1]**2)
                            for n in inner_corner_ids])
    R_INNER_LAYER = inner_radii.max()  # use max to avoid moving any inner face node
    R_OUTER_ORIG = new_r.max()

    print(f"\n  Boundary element layer: [{R_INNER_LAYER/1e3:.2f}, {R_OUTER_ORIG/1e3:.2f}] km")
    print(f"  Compressing to:         [{R_INNER_LAYER/1e3:.2f}, {R_TARGET/1e3:.1f}] km")

    # Linearly map [R_INNER_LAYER, R_OUTER_ORIG] -> [R_INNER_LAYER, R_TARGET]
    rescale_mask = new_r > R_INNER_LAYER
    if rescale_mask.any():
        old_r = new_r[rescale_mask]
        frac = (old_r - R_INNER_LAYER) / (R_OUTER_ORIG - R_INNER_LAYER)
        new_r_vals = R_INNER_LAYER + frac * (R_TARGET - R_INNER_LAYER)
        scale = new_r_vals / old_r
        new_coords[rescale_mask, 0] *= scale
        new_coords[rescale_mask, 1] *= scale
        num_rescaled = rescale_mask.sum()
        max_shift_km = (old_r - new_r_vals).max() / 1e3
        print(f"  Rescaled {num_rescaled} nodes (max shift: {max_shift_km:.2f} km)")

        # Recompute radii
        new_r = np.sqrt(new_coords[:, 0]**2 + new_coords[:, 1]**2)

    print(f"  Radius range after rescale: [{new_r.min()/1e3:.1f}, {new_r.max()/1e3:.1f}] km")
